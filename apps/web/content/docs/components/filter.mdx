...
The `filter-builder` block is the filter UI in the table's toolbar. The CLI copies it into `components/filter/`, or the folder you pass to `--path` (see [Getting Started](/docs/installation#notes-on---path)), so every file is yours to edit. The Radix UI, Base UI and React Aria versions have the same files and props.
...
| `setValue(value){:ts}` | Updates the draft. Raw text is fine; it's parsed on apply. |
