...
The `filter-builder` block is the filter UI in the table's toolbar. The CLI copies it into `components/filter/` (see [Getting Started](/docs/installation)), so every file is yours to edit. The Radix UI, Base UI and React Aria versions have the same files and props.
