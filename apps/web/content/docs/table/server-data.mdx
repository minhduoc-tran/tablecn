...
The [search](/docs/table/url-state#searching) looks in every column with an accessor, through the column's own accessor (so `customer.name` works). To narrow it, list the column ids:

```tsx title="Searched columns"
const searchColumns = ["id", "customer", "city"] // module level, or useMemo

useDataTable({ data: orders, columns, getRowId, searchColumns })
```

An id no column has is left out, with a warning in the console. If none of the ids match, every column is searched, so a typo doesn't empty the table.
