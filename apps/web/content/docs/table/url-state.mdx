...
| --- | --- | --- |
| `<NextFilterProvider>` or `useNextAdapter(){:ts}` | `@querycn/filter-next` | Next.js App Router |
| `useReactRouterAdapter(){:ts}` | `@querycn/filter-react/react-router` | react-router 6.4+ and 7 |
| `useBrowserUrlAdapter(){:ts}` | `@querycn/filter-react` | Vite and other apps without a client router |
| none | | State stays in memory, the URL is left alone. Good for dialogs and tests. |
| `createMemoryAdapter("status__eq=paid"){:ts}` | `@querycn/filter-react` | In memory, with a starting query string |

Without a `FilterProvider`, pass the adapter to `useDataTable({ adapter }){:ts}` instead.
...
`onApply` returns params to write together with the filter, so the filter and `page` change in one URL update. `resetPagePatch(){:ts}` returns `{ page: null }` (`null` removes a param), using your page param name if you renamed it. It also covers **Clear filters** and the chips' ×.
...
<Callout title="Already using ?q=?">
  If your app puts something else in `q` (a site-wide search box, say), the table would read it as its search. Rename the table's param: `url: { params: { search: "search" } }{:ts}`.
</Callout>
...
With your own TanStack table instead of `useDataTable`, `useTableUrlState({ adapter, …options }){:ts}` gives the URL state as controlled props: `sorting`, `pagination`, `search` and their `on…Change` handlers.
...
Create it once (module scope or `useState(() => …){:ts}`), not on every render. Its methods are called without `this`.
