---
title: URL state
description: How sorting, pagination and the search are written to the URL, next to the filter, and the options that shape them.
---

## Format

Sort, page and search live in the query string, next to the [filter's params](/docs/filter/url-state):

```txt title="URL"
/orders?status__eq=paid&q=nguyen&sort=-amount,customer_name&page=2&per_page=50
```

- `q` is the search box's text, trimmed and at most 200 characters. See [Searching](#searching).
...
flowchart LR
  ...
  search["Search box"] -- "q=nguyen<br/>page=null" --> url
...
- **Sort, page size or search.** The table does it itself, in the same write.
...
`q` holds free text from the [search box](/docs/table/components#datatablesearch). It's separate from the filter's rules: it isn't a chip, and it looks in several columns at once.

- **Client mode** keeps the rows where every word of the search appears in the searched columns, ignoring case and accents: `nguyen ha noi` finds "Nguyễn Văn An" in "Hà Nội". It looks in every column with an accessor, or only in [`searchColumns`](/docs/table/server-data#client-mode).
- **Server mode** sends it to the backend with the sort and page. The params presets put it where each backend expects it; see [Server data](/docs/table/server-data#searching).

The box writes the search once typing pauses (300 ms), or right away on <kbd>Enter</kbd>. From code, call `table.options.meta.setSearch("…")`, and read it from `table.options.meta.search`.

<Callout title="Already using ?q=?">
  If your app puts something else in `q` (a site-wide search box, say), the table would read it as its search. Rename the table's param: `url: { params: { search: "search" } }`. The same goes for `sort`, `page` and `per_page`.
</Callout>
...
- `q` longer than 200 characters is cut.
...
// → { sort: "-amount", page: "2", per_page: null, q: null }  (a ParamPatch: null removes the param)
```
...
// url.search and url.onSearchChange for a search box
```
