...
Choose with `useDataTable({ enableRowSelection }){:ts}`: `true`, or a function of the row. Other rows show a disabled checkbox.
...
const table = useDataTable({
  ...
  enableRowSelection: (row) => row.original.status !== "paid", // [!code highlight]
})
```
