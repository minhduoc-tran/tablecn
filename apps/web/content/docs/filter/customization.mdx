...
| Group | Contains |
| --- | --- |
| `operators` | A label per operator id: `eq: "is"{:ts}`, `between: "is between"{:ts}`. Add custom operators here. |
...
| `counts` | Functions: `selected(n){:ts}`, `more(n){:ts}`, `activeFilters(n){:ts}`, e.g. `activeFilters: (n) => n + " filters"{:ts}`. |
...
Outside React, `mergeMessages(enMessages, overrides){:ts}` gives the same merged object, and `getOperatorLabel(messages, operator, fieldType){:ts}` a label.
...
2. **Tell your serializer how to send it**:
   - `jsonApiSerializer` sends the id as is: `filter[amount][notBetween]=10&filter[amount][notBetween]=50`. Rename it with `operators: { notBetween: "not_between" }{:ts}`.
   - `djangoSerializer` needs a lookup: `lookups: { notBetween: "not_range" }{:ts}`, backed by a filter on your side.
   - `postgrestSerializer` needs an entry: `operators: { notBetween: (rule, quote) => … }{:ts}`.
