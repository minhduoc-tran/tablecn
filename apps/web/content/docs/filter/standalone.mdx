...
Anywhere inside the provider, `useAppliedFilter(){:ts}` gives you the request params for the applied filter:
...
`applyFilter(rows, state, context){:ts}` takes these options in `context`. `useAppliedFilter().context` already has `fields` and `registry`:
...
`createRowMatcher(state, context){:ts}` returns a `(row) => boolean` instead of a filtered array, for when you need a predicate.
...
- **Next.js 14**: `searchParams` is a plain object, not a Promise: `parseFilters(props.searchParams, { fields }){:ts}`. The examples use Next.js 15.5+ types like `PageProps<"/orders">`.
- `NextFilterProvider` takes every [`FilterProvider` prop](/docs/filter/react#filterprovider) except `adapter`, plus `shallow`. To use the adapter with your own provider, call `useNextAdapter({ shallow }){:ts}`. Both keep the path, hash and other params, don't scroll, and handle `basePath`.
