...
Operators the type doesn't have are ignored: `operators: ["eq", "between"]{:ts}` on a `text` field only offers `eq`.
...
- Results are cached for the page session. Call `clearFieldOptionsCache(field){:ts}` when the data changes (after creating a customer, on logout), or `clearFieldOptionsCache(){:ts}` to clear everything.
