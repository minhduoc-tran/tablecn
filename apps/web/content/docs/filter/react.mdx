...
`useRuleWarnings(rule)` returns the [warnings](/docs/filter/operators#warnings) for one draft rule, like `["reversedRange"]`. Get the text from `messages.warnings[key]`. It reads the draft, so use it in a small component of its own rather than in a memoized row.
