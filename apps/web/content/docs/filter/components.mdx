---
title: Components
description: The filter-builder block — what's in it, its props, and how to adapt it.
---

The `filter-builder` registry block is the UI from the [preview](/docs/filter). It's copied into your project (see [Installation](/docs/filter/installation#2-add-the-ui)), so every file is yours to edit. The Radix UI, Base UI and React Aria versions have the same files and props.
...
- Buttons have labels from [messages](/docs/filter/customization#messages-and-locales), e.g. "Remove filter: Status is Paid".
...
Each row renders the value input through a slot. The default, `FilterValueInput`, picks one by field type from `filterValueInputs`. To add an input for a [custom field type](/docs/filter/customization#custom-field-types), or replace a built-in one, pass your own map:
