...
The `filter-builder` registry block is the UI from the [preview](/docs/filter). It's copied into your project (see [Installation](/docs/filter/installation#1-add-the-ui)), so every file is yours to edit. The Radix UI, Base UI and React Aria versions have the same files and props.
